...
import { getLimitConfidence, listLimitAuditHistory } from "../../../src/db/repository.registry-tools";
...
import { listRelatedPublicLimits } from "../../../src/db/repository.related-limits";
...
export default async function LimitPage({ params }: PageProps) {
  ...
  const [research, verificationHistory, bounties, breakthroughEvents, specificationHistory, challengeHistory, confidence, auditHistory, relatedLimits] = database ? await Promise.all([getLimitResearchData(database.limit.id).catch(() => ({ specification: null, claims: [], evidence: [] })), listPublishedVerificationHistory(database.limit.id).catch(() => []), listPublicBounties(database.limit.id).catch(() => []), listBreakthroughEventsForLimit(database.limit.id).catch(() => []), getSpecificationVersionHistory(database.limit.id).catch(() => []), listPublicSubmissionHistory(database.limit.id).catch(() => []), getLimitConfidence(database.limit.id).catch(() => null), listLimitAuditHistory(database.limit.id).catch(() => []), listRelatedPublicLimits({ id: database.limit.id, category: database.limit.category, subcategory: database.limit.subcategory }).catch(() => [])]) : [null, [], [], [], [], [], null, [], []];
  ...
  return <main className="canonical-page">
    ...
    <PublicHeader />
    <Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Limits", href: "/open-limits" }, { label: record.title }]} />
    <section className="canonical-intro"><div className="canonical-category">{record.category}</div><div className="canonical-id-row"><span className="canonical-id">{record.id}</span><span className="canonical-status">{displayStatus.replaceAll("_", " ")} LIMIT</span></div><h1>{record.title}</h1><p>{record.summary}</p><div className="intro-foot">{database&&<PublicViewCount kind="LIMIT" target={record.id}/>}<span>Specification version {research?.specification?.version ?? 2}</span><span>{database ? `Published ${database.limit.publishedAt ? new Date(database.limit.publishedAt).toISOString().slice(0, 10) : undefined ?? "—"}` : "Last updated May 12, 2025"}</span><span>{research ? `${research.evidence.length} evidence records` : "19 cited papers"}</span></div><div className="canonical-share-actions"><a href={`/limits/${record.id}/opengraph-image`} target="_blank" rel="noreferrer">Generate share image ↗</a></div></section>
    <section className="problem-section" aria-labelledby="about-problem"><div className="section-title"><span>01</span><h2 id="about-problem">About this problem</h2><p>The question, scope, and sources behind this Registry record.</p></div><div className="problem-content"><div className="problem-description"><span className="problem-label">Description</span><p>{record.summary}</p></div><div className="problem-description"><span className="problem-label">Formal question</span><p>{formalQuestion}</p></div><div className="problem-metadata"><div><span className="problem-label">Scope &amp; constraints</span>{specificationConstraints.length ? <dl>{specificationConstraints.map(([key, value]) => <div key={key}><dt>{key.replaceAll("_", " ")}</dt><dd>{value}</dd></div>)}</dl> : <p>Current published specification applies.</p>}</div><div><span className="problem-label">Primary sources</span>{primarySources.length ? <ul>{primarySources.map((source) => <li key={source.id}><a href={source.sourceUrl} target="_blank" rel="noreferrer">{source.label} <span aria-hidden="true">↗</span></a>{source.location ? <small>{source.location}</small> : null}</li>)}</ul> : <p>No external source link is recorded for this legacy fixture.</p>}</div></div>{primarySources.some((source) => source.abstract) ? <div className="problem-abstracts">{primarySources.filter((source) => source.abstract).map((source) => <div key={source.id} className="problem-abstract"><span className="problem-label">Abstract</span><p>{source.abstract}</p><small>{source.label}</small></div>)}</div> : null}<ScopeCalculator constraints={scopeConstraintValues} /></div></section>

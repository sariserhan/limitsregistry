...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Sign-up never creates a session here (requireEmailVerification is on), so there's no
    // account to land the visitor in — router.push("/") used to send them to the homepage with
    // no indication anything else was needed, silently discarding the fact that their account
    // can't sign in until they click the link in their inbox.
    setCreated(true);
  }
  ...
  return <main className="auth-shell">
    ...
    {created ? <p>Almost there — we sent a verification link to {email}. Click it to activate your account, then sign in.</p> : <>

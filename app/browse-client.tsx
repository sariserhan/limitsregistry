...
export default function Home({ initialLimits, initialClaims }: BrowseClientProps) {
  ...
  const [status, setStatus] = useState<"ALL" | "OPEN" | "PROVEN">("ALL");
  ...
  const openCount = useMemo(() => limits.filter((limit) => limit.frontier.status === "OPEN").length, [limits]);
  const provenCount = limits.length - openCount;
  ...
  const filtered = useMemo(() => limits.filter((limit) => {
    ...
    const matchesStatus = status === "ALL" || limit.frontier.status === status;

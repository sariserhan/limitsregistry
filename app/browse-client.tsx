...
function BrowseFrontier({ limit }: { limit: PublishedLimit }) {
  ...
  if (presentation.mode === "ONE_SIDED") return <div className="bound-block bound-single"><span>{presentation.label}</span><strong>{formatExact(presentation.value)}</strong><p>{presentation.unknownLabel}</p></div>;

...
export default async function SponsorPage({searchParams}:{searchParams:Promise<{q?:string}>}) {
  const params=await searchParams;const q=typeof params.q==="string"?params.q.slice(0,200):"";
  const options=await listPublicLimitOptions(q,50);
  return <main className="submit-page"><PublicHeader/><div className="submit-content"><h1>Sponsor a research bounty</h1>
    <p>Support work on a specific Limit with a clearly labelled sponsor panel and a link to your organisation. We quote listing fees individually; the standard term is 90 days, nonexclusive, with no automatic renewal.</p>
    ...
    <EnquiryForm options={options.map(({id,registryNumber,title})=>({id,registryNumber,title}))}/>
  </div><SiteFooter/></main>;
}

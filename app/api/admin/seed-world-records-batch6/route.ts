...
import { WORLD_RECORDS_BATCH6 } from "../../../../src/catalog/world-records-batch6";
...
export async function POST(request: Request) {
  ...
  for (const record of WORLD_RECORDS_BATCH6) {
    ...
    try {
      await db.transaction(async (tx) => {
        ...
        const [spec] = await tx.insert(specificationVersions).values({
          ...
          assumptions: { kind: "OBSERVED_RECORD", recordType: "WORLD_RECORD", sourceUrl: record.source.url, publicationProcess: "WORLD_RECORDS_BATCH6_IMPORT" },
  ...
  return NextResponse.json({ inserted, skipped, errors, total: WORLD_RECORDS_BATCH6.length });
}

...
import { SPEED_RECORDS } from "../../../../src/catalog/speed-records";
...
export async function POST(request: Request) {
  ...
  for (const record of SPEED_RECORDS) {
    ...
    await db.transaction(async (tx) => {
      ...
      const [spec] = await tx.insert(specificationVersions).values({
        ...
        assumptions: { kind: "OBSERVED_RECORD", recordType: "WORLD_RECORD", sourceUrl: record.source.url, publicationProcess: "SPEED_RECORDS_IMPORT" },
      ...
      const [evidenceRow] = await tx.insert(evidence).values({
        ...
        limitId: limit.id, metadata: { verificationLevel: "SOURCE_CONFIRMED", value: record.value, unit: record.unit },
      ...
      await tx.insert(auditLogs).values([
        { action: "SPEED_RECORD_CLAIM_ACCEPTED", entityType: "CLAIM", entityId: claim.id, actorUserId: owner.id, after: { status: "ACCEPTED", value: record.value, unit: record.unit }, reason: `Verified against ${record.source.title}.` },
        { action: "SPEED_RECORD_LIMIT_PUBLISHED", entityType: "LIMIT", entityId: limit.id, actorUserId: owner.id, after: { status: record.limitStatus }, reason: `Source-backed publication from ${record.source.title}.` },
      ]);
    });
    inserted++;
  }

  return NextResponse.json({ inserted, skipped, total: SPEED_RECORDS.length });
}

...
import { WORLD_RECORDS_BATCH8_MATH_AI_ENERGY } from "../../../../src/catalog/world-records-batch8-math-ai-energy";
...
export async function POST(request: Request) {
  ...
  const errors: { registryNumber: string; error: string }[] = [];
  for (const record of WORLD_RECORDS_BATCH8_MATH_AI_ENERGY) {
    ...
    try {
      await db.transaction(async (tx) => {
        ...
        const [spec] = await tx.insert(specificationVersions).values({
          ...
          assumptions: { kind: "OBSERVED_RECORD", recordType: "WORLD_RECORD", sourceUrl: record.source.url, publicationProcess: "WORLD_RECORDS_BATCH8_MATH_AI_ENERGY_IMPORT" },
        ...
        await tx.insert(auditLogs).values([
          { action: "WORLD_RECORD_CLAIM_ACCEPTED", entityType: "CLAIM", entityId: claim.id, actorUserId: owner.id, after: { status: "ACCEPTED", value: record.value, unit: record.unit }, reason: `Verified against ${record.source.title}.` },
          { action: "WORLD_RECORD_LIMIT_PUBLISHED", entityType: "LIMIT", entityId: limit.id, actorUserId: owner.id, after: { status: record.limitStatus }, reason: `Source-backed publication from ${record.source.title}.` },
        ]);
      });
      inserted++;
    } catch (error) {
      errors.push({ registryNumber: record.registryNumber, error: error instanceof Error ? error.message : String(error) });
    }
  }

  return NextResponse.json({ inserted, skipped, errors, total: WORLD_RECORDS_BATCH8_MATH_AI_ENERGY.length });
}

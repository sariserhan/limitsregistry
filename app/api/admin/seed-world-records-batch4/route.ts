...
import { WORLD_RECORDS_BATCH4 } from "../../../../src/catalog/world-records-batch4";
...
export async function POST(request: Request) {
  ...
  for (const record of WORLD_RECORDS_BATCH4) {
    ...
    await db.transaction(async (tx) => {
      ...
      const [spec] = await tx.insert(specificationVersions).values({
        ...
        assumptions: { kind: "OBSERVED_RECORD", recordType: "WORLD_RECORD", sourceUrl: record.source.url, publicationProcess: "WORLD_RECORDS_BATCH4_IMPORT" },
  ...
  return NextResponse.json({ inserted, skipped, total: WORLD_RECORDS_BATCH4.length });
}

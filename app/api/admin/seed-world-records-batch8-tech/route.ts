...
import { WORLD_RECORDS_BATCH8_TECH } from "../../../../src/catalog/world-records-batch8-tech";
...
export async function POST(request: Request) {
  ...
  for (const record of WORLD_RECORDS_BATCH8_TECH) {
    ...
    try {
      await db.transaction(async (tx) => {
        ...
        const [spec] = await tx.insert(specificationVersions).values({
          ...
          assumptions: { kind: "OBSERVED_RECORD", recordType: "WORLD_RECORD", sourceUrl: record.source.url, publicationProcess: "WORLD_RECORDS_BATCH8_TECH_IMPORT" },
  ...
  return NextResponse.json({ inserted, skipped, errors, total: WORLD_RECORDS_BATCH8_TECH.length });
}

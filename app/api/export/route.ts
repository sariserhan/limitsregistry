...
export async function GET(request: Request) { if (!allowRequest(`export:${clientIp(request)}`)) return NextResponse.json({ error: "Rate limit exceeded." }, { status: 429 }); try { const data = await listPublishedDomainLimits(); const format = new URL(request.url).searchParams.get("format"); if (format === "csv") { const header = "registryNumber,title,category,status\n"; const rows = data.map(item => [item.id,item.title,item.category,item.status].map(csvCell).join(",")).join("\n"); return new Response(header + rows + "\n", { headers: { "content-type": "text/csv; charset=utf-8", "content-disposition": "attachment; filename=limits-registry.csv" } }); } return NextResponse.json({ data, generatedAt: new Date().toISOString() }, { headers: { "cache-control": "public, s-maxage=300, stale-while-revalidate=900" } }); } catch { return NextResponse.json({ error: "Registry export unavailable." }, { status: 503 }); } }

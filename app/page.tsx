...
export default async function Home() {
  ...
  const limits = databaseRows.length ? databaseRows.map((row) => {
    ...
    return { ...fallback, id: row.registryNumber, title: row.title, category: row.category, summary: row.summary, direction: row.direction, status: row.status === "PROVEN" ? "PROVEN" as const : "OPEN" as const };
  }) : publishedLimits;

  ...
  try {
    for (const record of chemistryRecords) {
      ...
      await sql.begin(async (tx) => {
        ...
        const [evidence] = await tx`insert into evidence(type, label, url, location, metadata) values (${"PAPER"}, ${`${record.source.title} — ${record.title}`}, ${record.source.url}, ${record.source.location}, ${tx.json({ doi: record.source.doi ?? null, formula: record.formula, verificationLevel: record.status === "PROVEN" ? "SOURCE_CONFIRMED" : "REPORTED", evidenceBasis: record.evidenceBasis })}) returning id`;

/** Publishes the six unsolved Clay Millennium Prize Problems and their real, verified $1M bounties.
 *  The seventh, the Poincaré conjecture, is solved (Perelman, 2003) and is deliberately excluded --
 *  there is no unclaimed prize to publish for it. */
...
async function main() { try {
  ...
  for (const p of PROBLEMS) {
    const existing = await sql`select id from limits where registry_number=${p.id} limit 1`;
    let limitId: string;
    if (existing.length) {
      limitId = existing[0].id;
      await sql`update limits set summary=${p.summary},updated_at=now() where id=${limitId}`;
      updated++;
    } else {
      const [limit] = await sql`insert into limits (registry_number,slug,title,summary,category,subcategory,direction,metric_name,status) values (${p.id},${p.id.toLowerCase().replace("lr-", "")},${p.title},${p.summary},${p.category},${p.subcategory},${"MAXIMIZE"},${"Resolution status"},${"OPEN"}) returning id`;
      limitId = limit.id;
      await sql`insert into limit_spec_versions (limit_id,version_number,formal_statement,constraints,assumptions) values (${limitId},1,${p.formalStatement},${sql.json({})},${sql.json({ publicationProcess: "FOUNDING_CATALOG_IMPORT" })})`;
      await sql`insert into evidence (type,label,url,limit_id,metadata) values (${"PAPER"},${`${p.title} — official problem description`},${p.sourceUrl},${limitId},${sql.json({ verificationLevel: "SOURCE_CONFIRMED" })})`;
      published++;
    }

...
async function main(){try{
  const queue=await sql`select l.id,l.registry_number,l.status,count(c.id)::int claim_count,count(ce.evidence_id)::int evidence_count from limits l join limit_spec_versions v on v.limit_id=l.id join claims c on c.specification_version_id=v.id left join claim_evidence ce on ce.claim_id=c.id where l.status=${"DRAFT"} group by l.id,l.registry_number,l.status order by l.registry_number`;
  const ineligible=queue.filter(row=>row.claim_count===0||row.evidence_count<row.claim_count);if(ineligible.length)throw new Error(`Evidence gate failed: ${ineligible.map(row=>row.registry_number).join(", ")}`);

...
describe("CODATA 2022 catalog snapshot",()=>{
  it("parses enough distinct source-backed constants for the 355-record target",()=>{const rows=parseCodataAscii(source);expect(rows.length).toBeGreaterThan(300);expect(new Set(rows.map(row=>row.quantity)).size).toBe(rows.length);expect(rows.slice(0,CODATA_DRAFT_COUNT)).toHaveLength(355);});

...
export async function exactPublicSearch(rawQuery: string, limit = 20) { const query = normalizeSearchQuery(rawQuery); if (!query) return []; const pattern = `%${query}%`; const [indexedRows, liveLimits] = await Promise.all([db.select({ entityType: semanticDocuments.entityType, entityId: semanticDocuments.entityId, registryNumber: semanticDocuments.registryNumber, title: semanticDocuments.title, content: semanticDocuments.content }).from(semanticDocuments).where(or(ilike(semanticDocuments.title, pattern), ilike(semanticDocuments.content, pattern))).limit(50), db.select({ entityId: limits.id, registryNumber: limits.registryNumber, title: limits.title, summary: limits.summary, category: limits.category }).from(limits).where(and(inArray(limits.status, ["OPEN", "PROVEN"]), or(ilike(limits.registryNumber, pattern), ilike(limits.title, pattern), ilike(limits.summary, pattern), ilike(limits.category, pattern)))).limit(50)]); const combined = [...indexedRows.map(row => mapResult({ ...row, score: exactRelevance(query, row.title, row.content) })), ...liveLimits.map(row => mapResult({ entityType: "LIMIT", entityId: row.entityId, registryNumber: row.registryNumber, title: row.title, content: `${row.registryNumber} ${row.category}\n${row.summary}`, score: exactRelevance(query, row.title, `${row.registryNumber} ${row.category} ${row.summary}`) }))]; const unique = [...new Map(combined.sort((a, b) => b.score - a.score).map(row => [`${row.entityType}:${row.entityId}`, row])).values()]; return keepPublicResults(unique, limit); }

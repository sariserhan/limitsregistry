...
export async function listBountiesByUser(userId: string) {
  return db.select({ bounty: researchBounties, limit: { registryNumber: limits.registryNumber, title: limits.title } })
    .from(researchBounties)
    .leftJoin(limits, eq(limits.id, researchBounties.limitId))
    .where(eq(researchBounties.submittedByUserId, userId))
...
export async function listPublicBounties(limitId?: string) {
  ...
  if(limitId)conditions.push(eq(limits.id,limitId));
  const rows=await db.select({bounty:publicBountyFields,limit:{id:limits.id,registryNumber:limits.registryNumber,title:limits.title}})
    .from(researchBounties).innerJoin(limits,bountyCoversLimit).where(and(...conditions)).orderBy(desc(researchBounties.verifiedAt));
  return [...new Map(rows.filter(({bounty})=>isPublicBounty(bounty.status,bounty.expiresAt)).map(row=>[row.bounty.id,row])).values()];
}
export async function listPublicBountyArchive() {
  const rows = await db.select({ bounty: publicBountyFields, limit: { id: limits.id, registryNumber: limits.registryNumber, title: limits.title } })
    .from(researchBounties).innerJoin(limits, bountyCoversLimit)
    .where(and(eq(researchBounties.status, "VERIFIED"), inArray(limits.status, ["OPEN", "PROVEN", "DISPUTED", "RETIRED"])))
    .orderBy(desc(researchBounties.createdAt));
  // A shared category award appears once in the archive, never once per covered record.
  return [...new Map(rows.map(row=>[row.bounty.id,row])).values()];
}
